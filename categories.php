...
<ol style='text-align:center' class="breadcrumb" >
    ...
    <?php
        if (isset($_GET['del_id'])) { //проверяем, есть ли переменная
            ...
            $sql = mysqli_query($link, "DELETE FROM category WHERE id = {$_GET['del_id']}");
            if ($sql) {
              echo "<center><p>Категория удалена.</p></center>";
...
<div class="container">
    ...
    <a href="add_categories.php" class="btn btn-primary">Добавить категорию</a>
    <hr>
    <?php if($result->num_rows > 0): ?>
        <ul>
            <?php while($value = $result->fetch_assoc()): ?>
                <li><a href="edit_categories.php?id=<?php echo $value["id"]; ?>">#<?php echo $value["id"]; ?> <?php echo $value["categoryname"]; ?></a> (<a onclick="return proverka();" style="color:red" href="?del_id=<?php echo $value["id"]; ?>">Удалить</a>)</li>
            <?php endwhile; ?>
        </ul>
    <?php else: ?>
