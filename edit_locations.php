<?php
    ...
    $info = mysqli_query($link, "SELECT * FROM `locations` WHERE id = '{$id} LIMIT 1';") or die(mysqli_error($link));
    ...
    if(!empty($_POST["ggo"])) {
        ...
        if(empty($name)) {
            ...
            mysqli_query($link, "UPDATE `locations` SET `location` = '{$name}' WHERE id = '{$id}';") or die(mysqli_error($link));
            echo "<script>location.href='locations.php'</script>";
        }
    }
?>
...
<div class="container text-center">
    ...
    <form class="form" method="POST" enctype="multipart/form-data" autocomplete="on">
        <label>
            ...
            <input type="text" name="name" class="form-control" value="<?php echo $info["location"]; ?>" style="font-weight: normal;" required />
        </label>
