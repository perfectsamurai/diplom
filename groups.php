<?php
    ...
    $result = mysqli_query($link, "SELECT * FROM `groups`;") or die(mysqli_error($link));
?>
<ol style='text-align:center' class="breadcrumb" >
    ...
    <?php
        if (isset($_GET['del_id'])) { //проверяем, есть ли переменная
            ...
            $sql = mysqli_query($link, "DELETE FROM `groups` WHERE id = {$_GET['del_id']}");
            if ($sql) {
              echo "<center><p>Группа удалена.</p></center>";
...
<div class="container">
    <h1>Список групп</h1>
    <a href="add_groups.php" class="btn btn-primary">Добавить группу</a>
    <hr>
    <?php if($result->num_rows > 0): ?>
        <ul>
            <?php while($value = $result->fetch_assoc()): ?>
                <li><a href="edit_groups.php?id=<?php echo $value["id"]; ?>">#<?php echo $value["id"]; ?> <?php echo $value["group"]; ?></a> (<a onclick="return proverka();" style="color:red" href="?del_id=<?php echo $value["id"]; ?>">Удалить</a>)</li>
            <?php endwhile; ?>
        </ul>
    <?php else: ?>
